...
$(function(){
    ...
    if(key != null){
        var urlSearch = DD_api.searchHealth,
            ...
            totalsSearch = resSearch.healthTotal;
        if(resSearch.flag == 'success'){
            var inner = '';
            for(var i=0;i<dataSearch.length;i++){
                var imgSearch =dataSearch[i].healthPhoto == null ? "" : dataSearch[i].healthPhoto;
                inner += '<a href="healthyDetails.html?id='+dataSearch[i].healthId+'" target="_blank" class="market_t_l_message"><div class="img fl"></div><div class="title fl"><h2>'+dataSearch[i].healthTitle+'</h2><p>'+dataSearch[i].healthIntroduce+'</p></div></a>';//<img src="'+imgSearch+'" alt="图片"/>
            }
        ...
        parentType.click(function(){
            var id = $(this).children().attr('id');
            window.location= 'healthy.html?parent='+id+'&page=1';  //   window.location= 'healthy.html?parent='+id+'&sun=&page=';
        });
    ...
    var DD = {
        'go':function(mm){
            if(key){
                ...
                window.location.href = 'healthy.html?parent='+parentId+'&page='+mm; // window.location.href = 'healthy.html?parent='+parentId+'&sun='+sunId+'&page='+mm;
            }
