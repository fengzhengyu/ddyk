...
//获取视频子类请求数据
function sunList(id,num){
    var url= DD_api.videoTypeList,
        list = {typeId:id,num:num},
...
function dataList(sunData,dataList1,pId,sunId){
    if(!sunData.length){
        ...
        for(var i=0;i<sunData.length;i++){
            sunVal += '<li>'+
                '<a href="videoDetails.html?id='+sunData[i].videoId+'" class="dataBox">'+
                '<div class="dataImg">'+
                '<img src="images/videoList.png" alt=""/>'+
                '<p class="dataInfo">我爱北京天那么</p>'+
                ...
                '<div class="dataTitle">'+
                '<p>有点萌！内蒙古学生骑马上学</p>'+
                '<span>网友：这就叫内蒙古STYLE</span></div></a></li>';
        }
